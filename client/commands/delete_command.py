...
def do_delete(shell, args):
    """Send a DELETE request to the server and prints the response."""
    ...
    if len(args) == 1:
        ...
        if "session" in shell._config.progConfig:
            ...
            response = shell._config.progConfig["session"].delete(drf_url, headers={"Authorization": f"Bearer {jwt_token}", "Accept": "application/json", "Content-Type": "application/json", })
        else:
