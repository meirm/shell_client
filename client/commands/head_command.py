...
def do_head(shell, args):
    """Send a HEAD request to the server and prints the response."""
    ...
    if len(args) == 1:
        ...
        if "session" in shell._config.progConfig:
            ...
            response = shell._config.progConfig["session"].head(drf_url, headers={"Authorization": f"Bearer {jwt_token}", "Accept": "application/json", "Content-Type": "application/json", })
        else:
