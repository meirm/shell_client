...
def do_options(shell, args):
    """Send a OPTIONS request to the server and prints the response."""
    ...
    if len(args) == 1:
        ...
        # send a OPTION request to the server as json
        ...
        if "session" in shell._config.progConfig:
            ...
            response = shell._config.progConfig["session"].options(drf_url, headers={"Authorization": f"Bearer {jwt_token}", "Accept": "application/json", "Content-Type": "application/json", })
        else:
