...
def do_post(shell, args):
    """Send a POST request to the server and prints the response."""
    clean = True
    show_headers = False
    if "verbose" in args:
        show_headers = True
    ...
    if data:
        if "session" in shell._config.progConfig:
            ...
            response = shell._config.progConfig["session"].post(url, data=data, headers={"Authorization": f"Bearer {jwt_token}", "Accept": "application/json", "Content-Type": "application/json" })
        else:
